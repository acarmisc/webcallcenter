<?php
class ImportaDati2 extends CI_Controller{
	...
	private function verificaCampiEssenziali($campi){
		...
		if(!in_array('ADV',$campi)) return false;
		if(!in_array('chiamati',$campi)) return false;
		if(!in_array('diretto',$campi)) return false;
		return true;
	}
	...
	private function aggiornaFarmacia($farmacia,$data_k){
		...
		if($data_k['chiamati']=='si'){
			$farmacia->flusso_lavoro='Contattato da callcenter';
		}else{
			$farmacia->flusso_lavoro='Non contattato da callcenter';
		}
	}
